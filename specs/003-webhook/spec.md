...
1. Given Censorr is running and a preset named "movies" exists in the configuration, when Radarr sends a webhook for a completed movie download that includes the tag key 'censorr_preset' with value 'movies', then the system acknowledges the webhook, enqueues a job, and records the decision in logs (processing happens asynchronously by a worker container).
...
6. Given a webhook contains at least one allowlisted tag (e.g., 'censorr_profile'), when the server evaluates the event, then it enqueues a job with the original payload for the worker to process according to 'censorr_preset' mapping and records the decision.
...
- FR-006: For qualifying events, the system MUST initiate exactly one processing job applying the identified preset to the referenced media item by enqueuing a job and returning 202 Accepted.
- FR-007: The system MUST NOT implement idempotency guarantees for duplicate webhook deliveries.
- FR-008: The system MUST record structured logs for each webhook: receipt time, decision (ignored/skipped/accepted), preset selected (if any), and job_id for traceability; the worker MUST record outcome logs tied to the job_id.
...
- FR-010: The system MUST fail gracefully and log a clear error when media is unavailable. The worker MUST support bounded retries for transient failures; validation/configuration failures MUST be treated as permanent without retry.
...
- FR-015: The system MUST treat the 'censorr_preset' key name as reserved by the product and MUST NOT allow end-users to remap or rename this key.
 ...
 - FR-024: The deployment MUST use separate container images with distinct Dockerfiles: a minimal webhook image (no ffmpeg) and a CLI/worker image (includes ffmpeg). The compose configuration MUST build them separately.
...
- FR-023: The system MUST require no additional services (e.g., Redis/DB) and use only the filesystem and standard library.
