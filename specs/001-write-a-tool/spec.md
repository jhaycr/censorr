...
21. TV REMUX_NEW_FILE: show folder tag policy: Given a TV episode and output mode REMUX_NEW_FILE with TV policy "subfolder_tag", when processing `TV/General/Only Murders in the Building/Season 1/S01E01.mkv`, then the remux is written to `TV/General/Only Murders in the Building [Censorr]/Season 1/S01E01.mkv` (creating the tagged show folder if necessary), and the original remains untouched.

22. TV REMUX_NEW_FILE: separate root policy: Given a TV episode and output mode REMUX_NEW_FILE with TV policy "separate_root" and root name `TV/Censorr`, when processing `TV/General/Only Murders in the Building/Season 1/S01E01.mkv`, then the remux is written to `TV/Censorr/Only Murders in the Building/Season 1/S01E01.mkv` (creating directories as needed), and the original remains untouched.
...
- FR-073 (TV new-file policies and templating): When output mode is `REMUX_NEW_FILE` for EPISODE media, the destination path MUST be computed by a configurable TV output policy. The system MUST support at least two policies: `subfolder_tag` (append a configurable tag, e.g., `[Censorr]`, to the show folder name) and `separate_root` (write under a configurable censored library root, e.g., `TV/Censorr`). The policy MUST be configurable globally and per preset, and MUST be reusable across shows via a template with tokens (e.g., `{library_root}/{show}{tag}/{season}/{episode}` for `subfolder_tag`). The system MUST create missing directories.
