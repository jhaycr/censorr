...
17. Preset config, movies: Given a default configuration file at `config/censorr.json` defines a preset named `movies`, when I run `censorr process "/data/media/movies/<movie>.mkv" --preset movies` with no other flags, then the tool executes the preset pipeline with operations `extract_subtitles,merge_subtitles,mask_subtitles,extract_audio,mute_audio,audio_quality_check,remux`, writes a sidecar masked subtitle, remuxes a new audio track alongside existing tracks (preserving originals), and replaces the original file in place (see backup behavior below). Language selection chooses non‑SDH/non‑CC English subtitles (by name/code or empty title), merges any English "Forced" track, and falls back to SDH/CC only when no non‑Forced English subtitles exist.

18. Preset config, tv: Given the preset named `tv`, when I run `censorr process "/data/media/tv/<show>/<episode>.mkv" --preset tv`, then the same pipeline as movies runs with the same language selection and outputs (sidecar subtitle next to the video; new muted audio track embedded). Episode outputs MUST NOT get a movie edition tag.
...
20. Backup on remux: Given the `--backup` option (or corresponding config), when the remux step replaces the original container in place, then a copy of the original file with `.bak` suffix is created in the same directory before replacement. When `--backup` is not set, no backup file is created by default.
...
- FR-069 (In-place remux and backup option): By default, the remux step SHOULD replace the original video in place after successful pipeline completion (consistent with FR‑058 final move when unset). When `--backup` is provided (or enabled in config), a copy `<filename>.bak` MUST be created next to the original before replacement. Replacement MUST be atomic where possible; on failure, the original MUST remain intact.
