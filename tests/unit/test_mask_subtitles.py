...
class TestMaskSubtitlesOperation:
    ...
    @patch('src.ops.subtitle_mask.SubtitleParser')
    @patch('src.ops.subtitle_mask.FuzzyMatcher')
    def test_run_with_profanity_filtering(self, mock_matcher_class, mock_parser_class):
        ...
        # Execute operation
        op = MaskSubtitlesOperation()
        # No QC mocking needed - QC is now a separate operation
        ...
        # Verify QC metadata is not present (QC is a separate operation now)
        assert "qc" not in result.metadata
    ...
    def test_verbose_mode(self, mock_matcher_class, mock_parser_class):
        ...
        # Check for any of the expected verbose messages (using new operation name)
        assert any("subtitle_mask" in call for call in print_calls)